...
void svm::initialize_q_matrix(double *x, int n) {
    q_matrix = new double[n*n];

    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            q_matrix[i*n+j] = dot_product_vectors(&x[i*n_features], &x[j*n_features], n_features);
...
int svm::update_alpha(double *x, int *y, int n) {
    int *up_ind = new int[n];
    int *lo_ind = new int[n]; 

    for (int i = 0; i < n; i++) {
        if ((alpha[i] < C && y[i] == 1) || (alpha[i] > 0 && y[i] == -1)) {
            up_ind[i] = 1;
        }
        else {
            up_ind[i] = 0;
    ...
    double *grad = dot_product_matrices(q_matrix, alpha, n, n, 1);
