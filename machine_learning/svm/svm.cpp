...
int *svm::predict(double *x, int n) {
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < n; i++) {
            if (scores[i] != INFINITY && scores1[i] != INFINITY) scores[i] += scores1[i];
        }
...
int svm::update_alpha(double *x, int *y, int n) {
    ...
    data[0] = (ind_0 != -1)?(double)ind_0+start:-1.0;
    data[1] = max_ind_0;
    data[2] = alpha_0;
    data[3] = grad_0;
    data[4] = (double) y_0;
    if (ind_0 != -1) data[5] = q_matrix[ind_0*n+ind_0+start];

    if (rank == 0) {
        MPI_Send(data, 6, MPI_DOUBLE, 1, 0, comm);
        MPI_Recv(data, 6, MPI_DOUBLE, n_process-1, 0, comm, MPI_STATUS_IGNORE);

        MPI_Send(data, 6, MPI_DOUBLE, 1, 1, comm);
        MPI_Recv(data, 6, MPI_DOUBLE, n_process-1, 1, comm, MPI_STATUS_IGNORE);
    }
    else {
        double *recv_data = new double[6];
        MPI_Recv(recv_data, 6, MPI_DOUBLE, (rank-1)%n_process, 0, comm, MPI_STATUS_IGNORE);

        if (recv_data[1] > max_ind_0) std::copy(recv_data, recv_data+6, data);

        MPI_Send(data, 6, MPI_DOUBLE, (rank+1)%n_process, 0, comm);
        MPI_Recv(data, 6, MPI_DOUBLE, (rank-1)%n_process, 1, comm, MPI_STATUS_IGNORE);
        MPI_Send(data, 6, MPI_DOUBLE, (rank+1)%n_process, 1, comm);
    }
    ...
    if (ind_0 != -1) {
        for (int i = 0; i < g; i++) {
            if (lo_ind[i] == 1) {
                ...
                if (u < max_ind_0) {
                    double p = q_matrix_00+q_matrix[i*n+i+start]-2.0*q_matrix[i*n+ind_0]/(y_0*y[i]);
                    p = (p >= 0.0)?p:1e-10;
    ...
    data = new double[8];
    data[0] = (ind_1 != -1)?(double)ind_1+start:-1.0;
    data[1] = min_ind_1;
    data[2] = alpha_1;
    data[3] = grad_1;
    data[4] = (double) y_1;
    data[5] = r_min;
    if (ind_1 != -1) data[6] = q_matrix[ind_1*n+ind_1+start];
    if (ind_1 != -1) data[7] = q_matrix[ind_1*n+ind_0];

    if (rank == 0) {
        MPI_Send(data, 8, MPI_DOUBLE, 1, 2, comm);
        MPI_Recv(data, 8, MPI_DOUBLE, n_process-1, 2, comm, MPI_STATUS_IGNORE);
        MPI_Send(data, 8, MPI_DOUBLE, 1, 3, comm);
        MPI_Recv(data, 8, MPI_DOUBLE, n_process-1, 3, comm, MPI_STATUS_IGNORE);
    }
    else {
        double *recv_data = new double[8];
        MPI_Recv(recv_data, 8, MPI_DOUBLE, (rank-1)%n_process, 2, comm, MPI_STATUS_IGNORE);

        if (recv_data[5] < r_min) std::copy(recv_data, recv_data+8, data);

        MPI_Send(data, 8, MPI_DOUBLE, (rank+1)%n_process, 2, comm);
        MPI_Recv(data, 8, MPI_DOUBLE, (rank-1)%n_process, 3, comm, MPI_STATUS_IGNORE);
        MPI_Send(data, 8, MPI_DOUBLE, (rank+1)%n_process, 3, comm);
    }
    ...
    grad_1 = data[3];
    y_1 = (int) data[4];
    double q_matrix_11 = data[6];
    double q_matrix_10 = data[7];
    ...
    if (ind_1 != -1 && max_ind_0-min_ind_1 > 1e-3) {
        ...
        double p = q_matrix_00+q_matrix_11-2.0*q_matrix_10/s;
        p = (p >= 0.0)?p:1e-10;
        ...
        for (int p = 1; p < n_process; p++) {
            if (p+1 <= m) {
                ...
                if (p <= m) {
                    ...
                    double *q_matrix_p = new double[u*n];
                    q_matrix_p = dot_product_matrices(x+(p*u+m)*n_features, xT, u, n_features, n);

                    for (int i = p*u+m; i < p*u+u+m; i++) {
                        for (int j = 0; j < n; j++) {
                            q_matrix_p[(i-p*u-m)*n+j] *= y[i]*y[j];
                        }
                    }

                    MPI_Isend(x+(p*u+m)*n_features, u*n_features, MPI_DOUBLE, p, p, comm, &request);
                    MPI_Isend(q_matrix_p, u*n, MPI_DOUBLE, p, p+1, comm, &request);
                    MPI_Isend(y+p*u+m, u, MPI_INT, p, p+2, comm, &request);
                }
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    svm v(n_features, max_iter, C, model_path, MPI_COMM_WORLD);
    v.fit(x, y, n);
